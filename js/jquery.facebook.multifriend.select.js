...
(function($) { 
    var JFMFS = function(element, options) {
        ...
        // ----------+----------+----------+----------+----------+----------+----------+
        elem.html(
            "<div class='jfmfs-plus-title'>"+settings.labels.title+"</div>"+
            ...
            "        <a class='filter-link' id='jfmfs-filter-cancel' href='javascript:close_(\"sf\");void(0);'>" + settings.labels.cancel + "</a>" +            
        ...
        this.getSelectedNames = function() {
            var names = [];
            $.each(elem.find(".jfmfs-friend.selected"), function(i, friend) {
                names.push( $(friend).find(".friend-name").text() );
            });
            return names;
        };        
